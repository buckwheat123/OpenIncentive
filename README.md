...
- **计划名按组合唯一（F5）**：同一**期间内（跨 BG 全局）**，一套独特的 `(KPI, Curve, 权重)` 组合只对应一个计划名（Quota 不计入）。导入时若「同组合、不同名」则**自动并入既有计划名并提示改名**；若「同名、不同组合」则**按传入顺序自动追加 `_v1` / `_v2` 后缀**。
...
- **加权支付率按原状加权加总、不归一化**：`加权支付率 = Σ(权重% × 支付率) ÷ 100`。当某计划 KPI 权重合计不是 100% 时，支付率随其等比缩放——这有时是正常/有意的，系统不改数值，仅记录当次计算的权重合计（`weight_total_pct`）并在导出 `comment` 列标注「本次计算权重不是100%」。
- **系统不提供未加权（简单平均）支付率**。
- **最终（季度总）支付率 = 加权支付率 + 特殊调整（±百分点）**。调整为增量而非覆盖，可正可负。
...
1. **季度大表导入**（后台 → 季度大表导入，BG 管理员亦可用）：一张表搞定人员 + 计划 + KPI + 实绩，每行一个 KPI。
...
8. **导出（两张表）**（后台 → 导出结果）：选择**年份**与 **BG**（均可不选即全部），两张表共用同一套筛选、页面上两个下载按钮：
   - **表一 · 总支付率**（`bonus_results*.csv`）：每次计算每人每计划一行。表头 `period,employee_id,name,bg,department,job_title,plan_name,weighted_rate_pct,weight_total_pct,adjustment_pct,final_rate_pct,adjusted,comment,calculated_at`。权重合计≠100% 时 `comment` 标注「本次计算权重不是100%」；**不含未加权支付率**。
   - **表二 · 各 KPI 明细**（`bonus_kpi_detail*.csv`）：每次计算每人每个 KPI 一行，取自计算时留存的 `detail_json`。表头 `period,employee_id,name,bg,department,job_title,plan_name,kpi_name,target,actual,curve_name,weight_pct,attainment_pct,rate_pct,calculated_at`。
...
10. **批量导入用户（仅平台管理员）**：在「用户管理」页下载模板（`employee_id,name,email,role,bg,department,job_title,manager_id,password`）→ 上传预览校验 → 确认执行。幂等 upsert：**工号不存在则新建**（`password` 留空即以工号为初始密码）、**已存在则只更新有变化的字段**、**与系统完全一致的行自动忽略**（可反复上传同一份花名册）。校验：必填、角色合法性（ADMIN/BG_ADMIN/MANAGER/EMPLOYEE）、表内工号或邮箱重复、邮箱被其他工号占用、上级工号须已存在。逐条留痕，报错行可下载 errors.csv。示例见 `sample_data/users_import.csv`。
...
- 达成率 = YTD 实绩 ÷ Quota × 100%；支付率 = Curve 分段线性插值（受封顶约束）；缺失实绩按 0 计并标注。
- 加权支付率 = Σ(权重% × 支付率) ÷ 100，**按原状加权加总、不归一化**。当某计划权重合计不是 100% 时，支付率随之等比缩放——这有时是正常/有意的，系统不改数值。
- **系统不提供未加权（简单平均）支付率**。计算时记录权重合计（`weight_total_pct`）；导出表一在权重合计≠100% 时于 `comment` 列标注「本次计算权重不是100%」。
- 季度总支付率（最终）= 加权支付率 + 特殊调整（±百分点，增量而非覆盖）。
- 内部支付率保留四位小数，展示与导出均以常规十进制呈现（避免科学计数法）。

## 测试与目录

离线引擎单测可直接运行；端到端测试需要一个**已启动的服务器**（会写入演示数据，测完请重新 `seed.py`）。Windows（git bash）顺序：

```bash
.venv/Scripts/python tests/test_calc.py        # 引擎单测（离线，含端到端计算、部分权重加总、区间斜率）
...
.venv/Scripts/python tests/test_e2e.py         # 端到端 33 步（需活服务器）
.venv/Scripts/python seed.py                    # 测完重新 seed，清掉测试污染的数据
```

端到端 33 步覆盖：季度大表导入 / 通知信数据导入（含年度格式、YTD 列展开、**F1 权重≠100% 整员工拒绝**、与季度计算冲突拦截与冲突清单导出）/ Curve / **一键计算** / **F2 封存后再算仍显示已计算** / 单人与批量调整 / 数据删除 / 封存 / 两张表导出与筛选 / **F6 指定季度预填大表导出** / **F5 计划名唯一化（改名与自动加后缀）** / BG 多对多与共管 / 代操作（proxy）/ **F4 模板全员共享 · 他人只读 · 复制 · 各自删除 / 平台 ADMIN 任意删除** 与计划-绩效两张表占位符 / 语言管理 / 中英切换 / 多层团队 / 四季度 / 批量导入用户 / 员工信息版本化封存 / 所辖 BG 重新同步 / 已阅追踪。
...
tests/          test_calc.py（离线引擎单测）+ test_e2e.py（端到端 33 步，需活服务器）
