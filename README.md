...
基于 Python 的线上奖金计算与沟通平台：**一张季度大表导入**（版本化、留痕、逐行校验、与系统一致则静默忽略）→ 管理员配置 Curve → 两步式触发计算 → 特殊调整（单人或 CSV 批量、增量、留痕）→ 数据删除（CSV、留痕）→ 封存与导出（按年份 + BG 筛选）→ 角色化查看（员工/多层经理/BG 管理员）→ **通知信数据单独导入** + 奖金通知信（模板 + HTML 邮件 + Curve 区间表 + 已阅追踪）。**平台管理员可在后台直接代操作（proxy）BG 管理员**。所有导入界面均可下载模板，并可从最近 3 个季度中选一个、按用户权限预填数据。默认中文界面，支持中/英切换，数据库字段值可通过「语言管理」维护翻译。
...
- **一张季度大表（v4）**：每季度的全部计算所需信息集中在一张 CSV 里上传，不再拆成员工/计划/实绩三张表。表头为 `period,employee_id,name,email,bg,department,job_title,manager_id,role,plan_name,kpi_name,weight_pct,quota,curve_name,actual`，**一行一个 KPI**；同一「期间 + 员工 + 计划名」的若干行构成一个计划。通知信数据用同样的表，但**去掉 `actual` 列**。
- **导入不再创建用户**：员工必须先在「用户管理」页建好；大表里的员工信息列只用于**更新已存在用户**的部门/职称/BG 等。表中出现系统里没有的员工即报错。
- **计划以 `plan_name` 为身份**（v3 已移除 `stip_type`）：大表**完整定义**该计划的 KPI 集合，导入即生成新版本并整体替换旧版本的 KPI。
- **逐行校验 + 一致则忽略（v4）**：上传先预览校验——用户是否存在、Curve 是否存在、表内是否有重复（同期间/人员/计划/KPI）、是否缺失计算必填项、是否落入已封存季度、BG 管理员是否越权；通过的行导入，出错的行给出**报错清单与汇总**（可下载 errors.csv）。**与系统内数据完全一致的行静默忽略、不报错、不生成新版本**。
...
- **软删除 + 留痕**：管理员可删除数据，但必须填写原因；删除逐条写入审计日志（`DataOpLog`），数据本身仅标记删除、不物理清除。代操作期间的导入会在审计原因中标注 proxy 来源。
...
4. **两步式触发计算**（后台 → 触发计算）：
   - 第一步：上传计算清单（表头 `period,employee_id,name,plan_name,action`，每行 `action` 填「计算」，可先按季度下载模板）→ 系统第一次读取并校验，展示预览（可计算 / 无计划 / 已封存 / 员工不存在）。
   - 第二步：确认 → 系统第二次读取并执行计算，生成批次（Run）。重复触发生成新批次，视图始终取最新批次。
5. **特殊支付率调整**（后台 → 特殊支付率调整）：
   - 单人：填写增量百分点（可负）与原因，即时生效。
   - **批量**：下载调整模板（`period,employee_id,name,adjustment_pct,reason`，可按季度预填）→ 上传先预览校验（成功/失败逐条标注）→ 确认后执行，逐条留痕；已封存范围自动跳过。
...
8. **导出**（后台 → 导出结果）：选择**年份**与 **BG**（均可不选即全部），分为**两张表**下载：
   - **表一 · 总支付率**（`bonus_results*.csv`）：每次计算每人每计划一行，含**加权支付率 / 权重合计 / 特殊调整 / 季度总支付率**；若某条数据在**当次计算时权重合计不是 100%**，该行 `comment` 列自动标注「本次计算权重不是100%」。不含未加权（简单平均）支付率。
   - **表二 · 各 KPI 明细**（`bonus_kpi_detail*.csv`）：每次计算每人每个 KPI 一行，含 `kpi_name / target / actual / curve_name / weight_pct / attainment_pct / rate_pct`，明细取自计算时留存的 `detail_json`。
   BG 管理员在 BG 视图同样可下载本 BG 的两张表（完整历史 + 各 KPI 明细）。
...
- **员工（我的奖金）**：按年度查看，Q1–Q4 四季度横向排布；未计算的季度留空但显示目标。每个计划（按计划名）展示各 KPI 的目标/达成率/支付率，以及 **加权支付率、特殊调整、季度总支付率**（不再展示未加权支付率）。年度下拉仅可选当前年度与上一年度。页头显示部门与职称。
- **经理（团队）**：可见向下最多 5 层的整个团队，每人标注相对层级（直属为 N-1，其下属为 N-2，依此类推）；支持按层级筛选（如只看 N-3）、按工号/总支付率排序，并显示团队平均总支付率。
- **BG 管理员（BG 视图）**：本 BG 全员当期结果（含部门/职称）与平均总支付率，可下载本 BG 的两张表（完整历史总支付率 + 各 KPI 明细）。
...
- 加权支付率 = Σ(权重% × 支付率) ÷ 100，按原状加权加总，**不做归一化**：权重合计不足或超过 100% 时支付率随其等比缩放（这有时是正常/有意的）。系统不提供未加权（简单平均）支付率。
- 计算时记录该计划的**权重合计**；导出结果时若某条数据当次计算的权重合计不是 100%，仅在该行 `comment` 列标注「本次计算权重不是100%」，支付率本身保持原样、不额外处理。
