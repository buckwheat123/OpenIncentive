...
8. **导出**（后台 → 导出结果）：选择**年份**与 **BG**（均可不选即全部），生成含**加权支付率 / 权重合计 / 调整 / 最终**费率的 CSV，并含每位员工当次计算的总加权支付率；若某条数据在**当次计算时权重合计不是 100%**，该行 `comment` 列自动标注「本次计算权重不是100%」。系统不再提供未加权（简单平均）支付率。BG 管理员仍下载本 BG 完整历史。
